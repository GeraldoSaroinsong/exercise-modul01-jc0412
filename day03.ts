...
let hasil6: string = "";
for (let i: number = 0; i <= kalimat6.length; i++) {
  ...
  if (i === 0 || charPrev2 === ".") {
    // check if the current character is in index 0 or if it's a new sentence, which is after a period and space
...
for (let i: number = 0; i < sent.length; i++) {
  ...
  if (swapChar === swapChar.toUpperCase()) {
    // if the current character is in uppercase, swap it to lowercase
    swapChar = swapChar.toLowerCase();
  } else if (swapChar === swapChar.toLowerCase()) {
    // if the current character is in lowercase, swap it to uppercase
...
let resultString = inputString.replace("a", "*"); //use a global flag (g) to change every letter in the sentence and not just the first one
